...
**4. [RESOLVED - was MED · ops] Partition-maintenance duty not scheduled.**
Stale as of the current code: `partition_maintenance` **is** now a real job (`cron/cron/jobs/partition_maintenance.py`) and **is** registered in the APScheduler worker (`cron/cron/scheduler.py:65`, monthly on the 1st at 01:00 Europe/Brussels), so `create_next_month_event_partitions()` has a scheduled caller and inserts dated after 2027-12 into `dispatch_lines`/`sales_events`/`restock_events` will not hit "no partition found". Nothing left to do here; keep the monthly job in the catalogue (it is one of the seven scheduled jobs).

...
- **F7 [EDGE] Stale fridge stock (BUILT).** A fridge's `fridge_stock` generation (`max(taken_at)` over its rows) is older than `STOCK_READING_MAX_AGE` (2h), or absent entirely. → Allocation still runs on that fridge's last-known units, but every `target_replenish` line for it is flagged `stock_stale=True` (exposed as `AllocationLineOut.stock_stale`) and a warning is logged. Staleness is decided **per fridge**, not table-wide. The guard lives in the allocation reader (`backend/app/services/menu_allocation_service.py`); it is NOT a cron alert, not a job status, not a health endpoint. Test: age a fridge's `taken_at` past 2h → flag set for that fridge only; fresh reading → flag clear.
- **F7b [EDGE] Single-fridge RFID offline (BUILT).** One fridge stops reporting while the others still do. → The `snapshot_stock` sweep is **scoped to the fridges present in the payload**, so the absent fridge is NOT swept: it retains its rows with their old `taken_at`. Its generation therefore ages out past 2h and F7 flags it `stock_stale=True`. It does **not** read as empty, and allocation does **not** over-dispatch it to full target. Test: run a snapshot whose payload omits fridge B → B's rows survive with the old generation and B is flagged stale, while fridge A's orphan rows are swept and A stays fresh.
- **F7c [EDGE] Product removed from a reporting fridge (BUILT).** Fridge A reports, but product X is no longer in it. → A is in the payload, so the sweep deletes X's stale row. X reads as `live = 0` against a **fresh** generation - correct, and NOT flagged stale (it is genuinely absent, not unknown).
...
- **N3 Sync health / failure.** A cron job fails. → `sync_run` row records failure; Sync screen shows red. Verify idempotent re-run (trailing overlap upsert produces no dupes). Note: **no alert/notification fires** - there is no `GET /health/jobs` endpoint and no alerting path (NOT BUILT, see G9).
...
| - | ~~**0004 Q10**~~ | ~~`stock_snapshot` retention (keep-all/downsample/prune)~~ | **RESOLVED - no longer a question.** The append-only snapshot log is replaced by the latest-only `fridge_stock` table (PK `(fridge_id, product_code)`, migration `0009_fridge_stock_latest_only.sql`), refreshed by `snapshot_stock` with mark-and-sweep in one transaction. Only the newest value per (fridge, product) was ever read, and fridge-stock history has **zero consumers** (no endpoint, no frontend, no report), so no history is retained and there is nothing left to retain/downsample/prune. Do not re-open this. |
...
| - | Lower | 0002 Q8 (UI language), Q12 (`job_runs`/`dispatch_line_versions` DDL - note **neither table exists**; `sync_run` already carries job/status/records/error/timings, so `job_runs` is likely redundant), Q13 (exact PA flow inventory), Q15/Q16 (Phase-5 scope, category mapping); 0003 Q1/Q2/Q4/Q8; 0004 Q2/Q6/Q8/Q11 | Real but deferrable or partly pre-answered by the brief (see 3c). |
...
- **G2 Data retention & GDPR.** Fridge-stock retention (old Q10) is moot now that `fridge_stock` is latest-only, but no policy exists for sales/restock event partitions, raw payload blobs, product reviews, or **customer personal data / customer_credit** (Belgium → GDPR). Retention + right-to-erasure unaddressed.
...
- **G9 Cron failure alerting.** `sync_run` records failures, but who gets paged/emailed when a sync, snapshot or backfill job fails is undefined. **No alert mail is configured anywhere** in the system: there is no email/digest path, no `alert_email_digest`, and the only alert row ever written is `negative_blocked` (`dispatch_service.py:399`). So there is no existing recipient list to piggyback on - this must be designed from zero.
...
- **Brief §13 fridge-stock cadence.** Brief mandates a **`fridge_stock` refresh every 15 min**, but 0004 Q3's options were only 30min/hourly/4×daily - 15 min is *finer than any option offered*. Storage is no longer an argument either way (the table is latest-only, mark-and-swept: it does not grow with cadence), so this is now purely a vendor-load vs freshness call. Still a decision the user was never actually shown; confirm 15 min vs the offered cadences.

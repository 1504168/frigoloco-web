...
- **Artifacts:** the architecture READMEs were built on `sync_cursors` (one row/feed, `last_synced_at`, "cursor advances only after commit") plus support tables `job_runs`, `backfill_checkpoints`, `live_stock_snapshot`, `generated_documents`, `reconciliation_daily` (six, per system-overview's "known delta"). **None of those six tables exist, and neither does `sync_cursors`.** **The actual code** (`backend/app/models/sync.py`, and `IMPLEMENTATION-BRIEF.md` #9) has **`sync_run` + `fridge_stock` only, and explicitly *no cursor table*** ("incremental jobs re-pull a trailing overlap window and upsert - no separate cursor table").
...
- **Artifacts:** `forecast.py`, `scoring.py`, `finance.py` are invoked on-demand by API routers **and** nightly by cron (`recompute_scores` - the only one of the three jobs originally cited here that exists; `auto_forecast` and `daily_husky_reconciliation` are **NOT BUILT**, and no reconciliation job of any kind exists). `CLAUDE.md` forbids code sharing across segments ("no API code outside `backend/`, no scheduled-job code outside `cron/`"). With the S1-1 dedicated-container decision, `cron/` cannot `import app.services.forecast` without depending on the backend image.
...
- **Artifact:** `recompute_scores` (daily 02:30, trailing-365-day, all products) over `sales_events`/`restock_events` at 20–30 M rows. (The nightly `daily_husky_reconciliation` this entry also cited is **NOT BUILT** - no reconciliation job exists - so today the contention is from `recompute_scores` alone.)
...
- **Fix:** state it once, unambiguously: "34 base tables in `schema.sql` + 50 pre-created monthly partition children + the 2 supplemental sync tables (`sync_run`, `fridge_stock`) = 36 relations total." Those two are the only bookkeeping tables in the system. Reference that line everywhere.
...
- **S4-5. [RESOLVED] Snacks/drinks allocation read fridge stock without a freshness gate.** The *finding* was right; the *reasoning as originally written was fabricated* and must not be copied: it claimed `below_target_alerts` "correctly skips on snapshot stale > 2h", but **`below_target_alerts` has never existed** (no such job, and no guard to copy from). Nor does `auto_forecast`: the allocation runs on request through `menu_allocation_service`, not from a Wed-15:00 cron. **Now fixed in the reader:** `backend/app/services/menu_allocation_service.py` applies `STOCK_READING_MAX_AGE = 2 hours` to each fridge's own `fridge_stock` generation (`max(taken_at)` over that fridge's rows); a stale or absent generation still allocates on last-known units but flags that fridge's `target_replenish` lines `stock_stale=True` (surfaced as `AllocationLineOut.stock_stale`) and logs a warning. **The single-fridge RFID-offline case is covered too:** the `snapshot_stock` sweep is scoped to the fridges present in the payload, so a fridge that stops reporting is not swept - it keeps its rows and its old generation, ages past 2h, and is flagged stale rather than reading as empty. (A global sweep would have wiped it, and allocation would have over-dispatched it to full target.) **Residual gap (NOT BUILT):** there is still no proactive *alert* - staleness is surfaced on the allocation response only; nothing pages anyone, and `rfid_offline_detector` does not exist.
...
| S2-1 | Maintainability | S2 | READMEs describe `sync_cursors`/6 support tables that do not exist; code+BRIEF use `sync_run`/`fridge_stock` only, no cursor |
...
| S4-1..6 | mixed | S4 | order-no tz, one-batch/date rigidity, matrix virtualization scale, type drift, tablet. **S4-5 RESOLVED**: per-fridge 2h `STOCK_READING_MAX_AGE` guard + `stock_stale` flag in `menu_allocation_service`, plus a payload-scoped sweep so an offline fridge is flagged stale rather than read as empty (its original premise - a guard copied from `below_target_alerts` - was fabricated; that job never existed). Residual: staleness is surfaced on the allocation response only, nothing alerts on it |

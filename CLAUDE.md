...
- `GET /{m}/stock/current` is **point-in-time only**: stock history cannot be backfilled. The `snapshot_stock` cron is time-critical for **current accuracy** (stale fridge stock means a wrong dispatch), not for history: fridge stock history is deliberately not retained (see the two stock concepts below).
- Beyond the deck's 4 endpoints, the API has 26 GET paths incl. `/fridge` (maps `if-XXXXXXX` hardware ids ↔ friendly names), `/fridgeproductprice` (per-fridge price overrides), and live device telemetry (`/fridge/{name}/state|temperature|products|tags`). The telemetry endpoints are a candidate source for an "RFID offline" alert, which is **NOT BUILT**: no code polls them, and no `rfid_offline` alert is ever raised (`AlertType` declares one, but `negative_blocked` is the only alert class actually constructed).
...
- **Forecast → Menu → Dispatch all key on (iso_year, week_no, day_name)** with import-from-previous-stage, save, load-saved, and explicit overwrite-confirm (delete + reinsert) semantics. Dispatch SAVE = planned (no stock effect); only "create individual dispatch" (is_dispatched) writes stock movements.
- **There are TWO stock concepts. Never conflate them.**
  - **Warehouse stock** is DERIVED, never stored: the `v_stock_balances` view computes it from `stock_movements` plus pending `purchase_order_lines`. It is per product with **no fridge dimension**, and it feeds the Stock page via `GET /stock/balances`.
  - **Fridge live stock** IS stored: the `fridge_stock` table holds the latest known in-fridge stock from Husky `GET /stock/current`, one row per `(fridge_id, product_code)` (that pair is the PK). It is the only source of fridge-level stock. **No history is kept and none is needed** - nothing ever read it, only the newest value per (fridge, product); do not re-open a retention/downsampling debate. Its **sole reader** is `backend/app/services/menu_allocation_service.py` (snacks/drinks branch, `restock = max(target_qty - live, 0)`); there is no API endpoint and no frontend that reads it.
  - **The `snapshot_stock` cron (every 15 min) refreshes `fridge_stock` by mark-and-sweep in one transaction, SCOPED TO THE FRIDGES THE PAYLOAD REPORTED ON.** It stamps one run-wide `taken_at` on the rows it upserts, then deletes the older-`taken_at` rows **of those fridges only**. So each fridge carries its own generation timestamp, and the three cases are all handled: (1) fridge reported, product gone -> that row is swept -> `live = 0` against a fresh generation, correct; (2) **fridge absent from the payload** (RFID reader offline, or a partial vendor response) -> its rows are deliberately **retained with their old `taken_at`**, which is the signal that flags it stale. Sweeping globally would instead wipe those fridges, and allocation would read them as empty and over-dispatch every one to full target; (3) an EMPTY payload never sweeps at all (that means a vendor problem, not empty fridges).
  - **Staleness is per fridge, not global.** If a fridge's generation is absent or older than `STOCK_READING_MAX_AGE` (2 h), allocation still runs on that fridge's last-known units - quantities are never silently changed - but flags its `target_replenish` lines `stock_stale=True` (surfaced as `AllocationLineOut.stock_stale`) and logs a warning. That 2 h guard lives in the allocation reader only: it is NOT a cron alert, NOT a job status, NOT a health endpoint.
